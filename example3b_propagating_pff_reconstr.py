...
physics_pars = {'E': 1, 'nu_s': 0.3, 'pre': 4e-2, 'pre_0': 0, 'alpha_B': 0,
                'theta_0': 0.0, 'theta': 0, 'alpha_th': 1e-5, 'rho_f': 1,
                'nu_f': 0.001, 'kappa_th': {'fluid': 0.005, 'solid': 1},
                'gravity': 0.5}

rhs_data_fluid = CF((0.02 * exp(-400 * ((x - 0.1)**2)), 0))
rhs_data_tmp = - 800 * exp(-(x**2 + y**2))

hash_pars = hashlib.blake2b(str(physics_pars).encode('utf-8'), digest_size=6).hexdigest()
filename = f'results/example3b_propagating_pff_h{hmax}eps{pf_eps}_{hash_pars}'


def p_func(n):
    return CF(4e-2 + n * 5e-5)
...
with TaskManager():
    ...
    for it in range(n_steps):
        ...
        # Step 3: Solve TFSI Problem on new mesh
        ...
        physics_pars['pre'] = p_func(it) + gfu_tfsi.components[1]
        ...
        if vtk_out:
            vtk = VTKOutput(
                ma=mesh, coefs=[*gfu_tfsi.components, *pf_solver.gfu.components],
